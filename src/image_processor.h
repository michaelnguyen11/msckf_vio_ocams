...
public:
  ...
  void start(StereoData &camData, ImuBuffer &imuBuffer, const ImageProcessorOutput::Ptr &output);
...
private:
   ...
   *    stereo images.
   */
  void initializeFirstFrame(const StereoData &camData);

  /*
   * @brief trackFeatures
   *    Tracker features on the newly received stereo images.
   */
  void trackFeatures(const StereoData &camData, ImuBuffer &imu_msg_buffer);
   ...
   */
  void addNewFeatures(const StereoData &camData);
   ...
   */
  void publish(const ImageProcessorOutput::Ptr &output);
  ...
  void integrateImuData(cv::Matx33f& cam0_R_p_c,
      cv::Matx33f& cam1_R_p_c, ImuBuffer &imu_msg_buffer, const StereoData &currCamData);
  ...
  void removeUnmarkedElements(
    ...
    if (raw_vec.size() != markers.size()) {
      LOG(WARNING) << "The input size of raw_vec: " << raw_vec.size() << " and markers : " << markers.size() << " does not match...";
    }
